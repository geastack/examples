...
import {
  createGeaWebGLRenderer,
  isGeaWebGLRendererReady,
  logAngleHostSmoke,
  syncGeaWebGLRendererSize,
} from './angleHost'
import {
  startThreeFrameLoop,
} from '#three-scene-source'

export function mountThreeAngleMetal(): void {
  logAngleHostSmoke("mounting apple-native Three ANGLE Metal probe")
  ...
  logAngleHostSmoke("attaching ANGLE Metal host")
  console.error("[three-angle-metal] attaching ANGLE Metal host")
  const renderer = createGeaWebGLRenderer(stageView, width, height, 2)
  const hostReady = isGeaWebGLRendererReady(renderer)
  logAngleHostSmoke(hostReady ? "ANGLE Metal host ready" : "ANGLE Metal host unavailable")
  ...
  syncGeaWebGLRendererSize(renderer, aspect)
  let frameCount = 0
  let fpsFrameCount = 0
  ...
  startThreeFrameLoop(renderer, aspect)
}
