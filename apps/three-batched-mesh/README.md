...
Run everything from this directory. The validation runner, the plugin and the
comparison script come from `@geastack/native-webgl-angle`, which this app
depends on. The runner is not in the published package, only in that package's
repository, so point node at your checkout of it:
...
This builds, launches, captures and compares the native renderer's output, then
...
batched instances, then one after hiding. Multidraw is not used, so the draw
count does not drop. The standard plugin continues to reject BatchedMesh
